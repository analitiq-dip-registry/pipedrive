...
- **Do not manually edit** the `version` field in `definition/connector.json` or `CHANGELOG.md` version entries — a GitHub Action updates these automatically when the PR is merged
...
1. Bumps the `version` field in `definition/connector.json` according to the label
